...
export default function PortfolioReportsPage() {
  ...
  function printReport() {
    ...
    // Convert markdown to HTML for clean printing
    const lines = selectedReport.content.split('\n')
    const bodyHtml = lines.map((line: string) => {
      ...
      if (line.startsWith('| ')) {
        ...
        if (isSep) return ''
        return `<tr>${cells.map((c: string) => `<td>${c.trim().replace(/\*\*(.+?)\*\*/g,'<strong>$1</strong>')}</td>`).join('')}</tr>`
      }
    ...
    // Wrap table rows
    .replace(/(<tr>.*?<\/tr>\n?)+/gs, (match: string) => `<table>${match}</table>`)

    const html = \`<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <title>\${portfolio?.name} — \${selectedReport.report_type} Report</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body { font-family: 'Segoe UI', Arial, sans-serif; color: #111; font-size: 10.5pt; line-height: 1.7; background: white; }
    .page-header { background: #0f1923; color: white; padding: 24px 40px; border-bottom: 3px solid #c9a84c; margin-bottom: 32px; }
    ...
    .report-meta { font-size: 11pt; color: #8a91a8; }
    .content { padding: 0 40px 40px; }
    h2 { font-size: 12pt; font-weight: 700; text-transform: uppercase; letter-spacing: 0.06em; color: #0f1923; border-bottom: 1px solid #c9a84c40; padding-bottom: 6px; margin: 28px 0 10px; }
    h3 { font-size: 11pt; font-weight: 600; color: #5b21b6; margin: 18px 0 8px; }
    p { margin: 6px 0; color: #222; }
    strong { color: #0f1923; font-weight: 700; }
    em { color: #555; font-style: italic; }
    code { background: #f5f5f5; padding: 1px 4px; border-radius: 3px; font-size: 9.5pt; font-family: monospace; }
    hr { border: none; border-top: 1px solid #ddd; margin: 20px 0; }
    table { width: 100%; border-collapse: collapse; margin: 12px 0; font-size: 9.5pt; }
    td { padding: 6px 8px; border-bottom: 1px solid #eee; vertical-align: top; }
    tr:first-child td { font-weight: 700; background: #f9f9f9; border-bottom: 2px solid #ddd; }
    .print-btn { position: fixed; top: 16px; right: 16px; background: #a78bfa; color: white; border: none; border-radius: 8px; padding: 10px 20px; font-size: 12px; font-weight: 600; cursor: pointer; font-family: inherit; }
    @media print {
      @page { size: A4; margin: 15mm; }
      .print-btn { display: none !important; }
      .page-header { background: none !important; border-bottom: 2px solid #c9a84c; padding: 0 0 12px; margin-bottom: 20px; }
      .firm { color: #c9a84c !important; }
      .port-name { color: #0f1923 !important; font-size: 16pt; }
...
<body>
  ...
  <div class="page-header">
    ...
    <div class="port-name">\${portfolio?.name}</div>
    <div class="report-meta">\${selectedReport.report_type.toUpperCase()} REPORT &nbsp;·&nbsp; \${selectedReport.report_date} &nbsp;·&nbsp; Generated: \${date}</div>
  </div>
