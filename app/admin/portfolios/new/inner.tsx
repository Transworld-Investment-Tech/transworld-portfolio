...
export default function NewPortfolioPageInner() {
  ...
  const [form, setForm] = useState({
    ...
    label: 'A', name: '', starting_nav: '',
    start_date: new Date().toISOString().slice(0, 10),
    currency: 'NGN', income_target: '15', cap_target: '30',
    liq_min: '5', dd_alert: '7', dd_action: '10',
    max_eq_single: '7', max_eq_sleeve: '35', notes: '',
    seedHoldings: true,
  })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (Math.abs(total - 100) > 0.01) { setError(`Sleeve targets must sum to 100%. Currently: ${total}%`); setSaving(false); return }
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="tw-card space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs text-[#8a91a8] mb-1.5">Label <span className="text-[#ff5c7a]">*</span></label>
            ...
            <div>
              ...
              <input value={form.name} onChange={set('name')} className="tw-input" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs text-[#8a91a8] mb-1.5">Starting NAV (₦) <span className="text-[#ff5c7a]">*</span></label>
                <input type="number" value={form.starting_nav} onChange={set('starting_nav')} required min="1" className="tw-input font-mono" />
              </div>
              <div>
          ...
          <div className="tw-card space-y-4">
            <div className="text-xs font-semibold uppercase tracking-widest text-[#555d72] pb-2 border-b border-white/[0.07]">Targets & thresholds</div>
            <div className="grid grid-cols-2 gap-4">
              {[['Income target (%)', 'income_target'],['Cap. target (%)', 'cap_target'],['Max single equity (%)', 'max_eq_single'],['Max equity sleeve (%)', 'max_eq_sleeve'],['Drawdown alert (%)', 'dd_alert'],['Drawdown action (%)', 'dd_action']].map(([label, key]) => (
                <div key={key}>
                  <label className="block text-xs text-[#8a91a8] mb-1.5">{label}</label>
                  <input type="number" value={(form as any)[key]} onChange={set(key)} step="0.1" min="0" max="100" className="tw-input" />
                </div>
              ))}
            </div>
          </div>
          <div className="tw-card space-y-4">
            <div className="flex items-center justify-between pb-2 border-b border-white/[0.07]">
              <div className="text-xs font-semibold uppercase tracking-widest text-[#555d72]">Sleeve targets</div>
              <span className={`text-xs font-mono ${Math.abs(sleeveTotal - 100) < 0.01 ? 'text-[#00d4a4]' : 'text-[#ff5c7a]'}`}>Total: {sleeveTotal}%</span>
            </div>
            {sleeves.map((s, i) => (
              <div key={s.sleeve_id} className="grid grid-cols-4 gap-3 items-end">
                <div><div className="text-xs text-[#8a91a8]">{s.name}</div></div>
                {[['Target %', 'target_pct'], ['Min %', 'min_pct'], ['Max %', 'max_pct']].map(([lbl, key]) => (
                  <div key={key}>
                    <label className="block text-[10px] text-[#555d72] mb-1">{lbl}</label>
                    <input type="number" value={(s as any)[key]} onChange={e => updateSleeve(i, key, e.target.value)} step="1" min="0" max="100" className="tw-input py-1.5 text-xs font-mono" />
                  </div>
                ))}
              </div>
            ))}
          ...
          <div className="tw-card">
            <label className="flex items-start gap-3 cursor-pointer">
              <input type="checkbox" checked={form.seedHoldings} onChange={e => setForm(f => ({ ...f, seedHoldings: e.target.checked }))} className="mt-0.5 accent-[#a78bfa]" />
              <div>
                <div className="text-sm font-medium">Seed default holdings</div>
                <div className="text-xs text-[#555d72] mt-0.5">Pre-populate all 12 instruments at NAV-proportional sizes.</div>
              </div>
            </label>
          </div>
          {error && <div className="alert alert-critical">{error}</div>}
          <div className="flex gap-3">
            <button type="submit" disabled={saving} className="flex items-center gap-2 bg-[#a78bfa] text-white px-5 py-2.5 rounded-lg text-sm font-medium hover:bg-[#9b87e8] disabled:opacity-50 transition-colors">
              <Save size={14} /> {saving ? 'Creating…' : 'Create portfolio'}
            </button>
            <Link href="/" className="flex items-center gap-2 border border-white/10 px-5 py-2.5 rounded-lg text-sm text-[#8a91a8] hover:text-[#e8eaf0] transition-colors">Cancel</Link>
          </div>
        </form>
      </div>
    </div>
