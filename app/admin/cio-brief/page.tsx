...
// v21r-hotfix-1: Comprehensive renderer improvements:
//   1. normalizeParagraphs() joins broken lines (AI wraps at ~80 chars)
//   2. Block-based parser handles h2/h3/h4/table/blockquote/bullet/paragraph
//   3. Proper <table> elements with navy header + zebra rows
//   4. Blockquotes rendered as gold KPI callout cards
//   5. Full justify on all prose paragraphs
//   6. Print HTML updated to match
...
// ─── Join lines broken mid-paragraph by the AI ───────────────────────────────
...
function renderMarkdown(raw: string): React.ReactNode[] {
  return parseBlocks(raw).flatMap((block, i): React.ReactNode[] => {
    switch (block.t) {
      case 'h2':
        return [<h2 key={i} className="hybrid-serif" style={{ fontSize: 21, fontWeight: 500, color: 'var(--text)', marginTop: 36, marginBottom: 4, borderBottom: '2px solid var(--gold)', paddingBottom: 8, letterSpacing: '-0.005em' }}>{block.text}</h2>]
      case 'h3':
        return [<h3 key={i} className="hybrid-serif" style={{ fontStyle: 'italic', fontSize: 16, fontWeight: 500, color: 'var(--gold)', marginTop: 26, marginBottom: 6 }}>{block.text}</h3>]
      case 'h4':
        return [<h4 key={i} style={{ fontSize: 12, fontWeight: 700, color: 'var(--text)', textTransform: 'uppercase' as const, letterSpacing: '0.1em', marginTop: 22, marginBottom: 5, borderLeft: '3px solid var(--gold)', paddingLeft: 10 }}>{block.text}</h4>]
      case 'hr':
        return [<hr key={i} style={{ border: 'none', borderTop: '1px solid var(--border-soft)', margin: '30px 0' }} />]
      case 'blank':
        return [<div key={i} style={{ height: 8 }} />]
      case 'p':
        return [<p key={i} style={{ fontSize: 13.5, lineHeight: 1.9, color: 'var(--text)', margin: '8px 0', textAlign: 'justify' as const }}>{formatInline(block.text)}</p>]
      case 'li':
        return [
          <div key={i} style={{ display: 'flex', gap: 10, margin: '5px 0 5px 10px' }}>
            <span style={{ color: 'var(--gold)', flexShrink: 0, fontWeight: 700, marginTop: 2 }}>\u00b7</span>
            <p style={{ fontSize: 13.5, lineHeight: 1.85, color: 'var(--text)', margin: 0, textAlign: 'justify' as const }}>{formatInline(block.text)}</p>
          </div>
        ]
      case 'bq':
        return [
          <div key={i} style={{ borderLeft: '3px solid var(--gold)', background: 'linear-gradient(90deg, rgba(176,139,62,0.08) 0%, rgba(176,139,62,0.03) 100%)', padding: '10px 16px', margin: '5px 0', borderRadius: '0 4px 4px 0' }}>
            <p style={{ fontSize: 13, color: 'var(--text)', margin: 0, fontWeight: 500 }}>{formatInline(block.text)}</p>
          </div>
        ]
      case 'table':
        return [
          <div key={i} style={{ overflowX: 'auto', margin: '20px 0', borderRadius: 4, border: '1px solid var(--border)', boxShadow: '0 1px 4px rgba(15,41,71,0.06)' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12.5 }}>
              {block.headers.length > 0 && (
                <thead>
                  <tr style={{ background: 'var(--sidebar-bg)' }}>
                    {block.headers.map((h, j) => (
                      <th key={j} style={{ padding: '10px 14px', textAlign: 'left' as const, color: 'var(--gold-bright)', fontWeight: 600, fontSize: 10, textTransform: 'uppercase' as const, letterSpacing: '0.14em', whiteSpace: 'nowrap' as const, borderRight: j < block.headers.length - 1 ? '1px solid rgba(232,217,181,0.1)' : 'none' }}>
                        {h}
                      </th>
                    ))}
                  </tr>
              ...
              <tbody>
                {block.rows.map((row, ri) => (
                  <tr key={ri} style={{ background: ri % 2 === 0 ? 'var(--bg-soft)' : 'var(--card)' }}
                    onMouseEnter={e => { e.currentTarget.style.background = 'var(--gold-soft)' }}
                    onMouseLeave={e => { e.currentTarget.style.background = ri % 2 === 0 ? 'var(--bg-soft)' : 'var(--card)' }}
                  >
                    {row.map((cell, ci) => (
                      <td key={ci} style={{ padding: '9px 14px', borderBottom: ri < block.rows.length - 1 ? '1px solid var(--border-soft)' : 'none', borderRight: ci < row.length - 1 ? '1px solid var(--border-soft)' : 'none', color: ci === 0 ? 'var(--text)' : 'var(--text-2)', fontWeight: ci === 0 ? 500 : 400 }}>
                        {formatInline(cell)}
                      </td>
                    ))}
                  </tr>
...
function buildPrintHTML(briefDate: string, content: string): string {
  ...
  const css = "@import url('https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,400;0,500;0,600;1,500&family=DM+Sans:wght@400;500;600&display=swap');"
    ...
    + '.content{max-width:740px;margin:0 auto;padding:36px 52px 60px;background:#fffbf2}'
    ...
    + 'hr{border:none;border-top:1px solid rgba(15,41,71,0.12);margin:24px 0}'
    + '.gap{height:8px}'
    ...
    + 'thead tr{background:#0a1f3a}'
    + 'th{padding:9px 12px;text-align:left;color:#c9a556;font-weight:600;font-size:8pt;text-transform:uppercase;letter-spacing:0.12em;border-right:1px solid rgba(232,217,181,0.1)}'
    + 'td{padding:8px 12px;border-bottom:1px solid rgba(15,41,71,0.07);border-right:1px solid rgba(15,41,71,0.06)}'
    + 'tr.even td{background:#faf5ea}tr.odd td{background:#fffbf2}'
    + 'td.first{font-weight:600;color:#0f2947}'
    ...
    + '@media print{@page{size:A4;margin:15mm}.pbtn{display:none!important}body{background:white}'
    + '.ph{background:none!important;border-bottom:2px solid #b08b3e;padding:0 0 18px;margin-bottom:24px}'
    + '.title{color:#0f2947!important}.firm{color:#b08b3e!important}.meta{color:#5c6573!important}'
    + '.content{padding:0}thead tr{background:#0a1f3a!important;-webkit-print-color-adjust:exact;print-color-adjust:exact}}'
    ...
    <div className="hybrid-page" style={{ display: 'flex', height: '100%', overflow: 'hidden' }}>
      ...
      <aside style={{ width: 276, flexShrink: 0, background: 'var(--card)', borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '16px 18px', borderBottom: '1px solid var(--border-soft)' }}>
          ...
          {generating
            ? <p style={{ marginTop: 10, fontSize: 10, color: 'var(--warn)', lineHeight: 1.5, fontWeight: 500 }}>&#8987; Searching live market data and synthesising &#8212; please wait, do not close this tab.</p>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          <div style={{ padding: '10px 18px 6px', fontSize: 9, fontWeight: 600, textTransform: 'uppercase' as const, letterSpacing: '0.18em', color: 'var(--text-3)' }}>History ({history.length})</div>
          ...
          {history.map(b => {
            ...
            return (
              <div key={b.id} onClick={() => setSelected(b)}
                style={{ padding: '11px 18px', cursor: 'pointer', borderBottom: '1px solid var(--border-soft)', background: isSel ? 'var(--gold-soft)' : 'transparent', borderLeft: isSel ? '2px solid var(--gold)' : '2px solid transparent', transition: 'background 0.15s' }}
              ...
              <div style={{ maxWidth: 900, margin: '0 auto', background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 5, padding: '44px 56px 52px' }}>
                ...
                <div style={{ marginTop: 48, padding: '16px 20px', background: 'var(--gold-soft)', border: '1px solid rgba(176,139,62,0.2)', borderRadius: 4 }}>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--text-2)', lineHeight: 1.65 }}>
                    Print this brief and use each section as a natural call segment. Market Overview opens the call; Outlook closes it. The Key Holdings Intelligence tables give you the specific numbers clients will ask about.
                  </div>
                </div>
              </div>
            </div>
