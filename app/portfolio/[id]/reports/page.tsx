      ...
      <div className="w-72 border-r border-white/[0.07] flex flex-col flex-shrink-0 bg-[#13161d]">
        ...
        <div className="p-5 border-b border-white/[0.07]">
          ...
          <button onClick={generate} disabled={generating}
            ...
            {generating
              ? <><div className="w-3 h-3 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Searching & writing…</>
          ...
          <div className="mt-3 text-[10px] text-[#555d72] leading-relaxed">
            Claude searches live NGX prices, P/E ratios, earnings results, dividend announcements, CBN rates, and macro data — then generates a fully formatted HTML report with investment signals.
          </div>
        </div>

        {/* History list */}
        <div className="flex-1 overflow-y-auto">
          <div className="px-5 py-3 text-[10px] font-bold uppercase tracking-widest text-[#555d72] flex items-center justify-between">
            ...
            history.map(r => {
              ...
              return (
                <div key={r.id} onClick={() => setSelectedReport(r)}
                  className={`px-5 py-3 cursor-pointer border-b border-white/[0.05] transition-colors ${selectedReport?.id === r.id ? 'bg-[#a78bfa]/[0.07]' : 'hover:bg-white/[0.02]'}`}>
        ...
        {generating ? (
          <div className="flex-1 flex flex-col items-center justify-center gap-5 text-[#555d72]">
            ...
            <div className="text-center">
              ...
              <div className="text-xs mt-1" style={{ color: ['quarterly','annual'].includes(reportType) ? '#f59e0b' : '#555d72' }}>
                {reportType === 'annual'    ? '⏳ Annual reports take 60–120 seconds — please wait patiently' :
                 reportType === 'quarterly' ? '⏳ Quarterly reports take 45–90 seconds — please wait' :
          ...
          <>
            ...
            <div className="px-6 py-3 border-b border-white/[0.07] bg-[#13161d] flex items-center justify-between flex-shrink-0">
              <div className="flex items-center gap-3">
                <div>
                  ...
                  <span className="ml-2 text-[11px] capitalize font-medium px-2 py-0.5 rounded"
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => downloadHTML(selectedReport)}
                  className="flex items-center gap-1.5 px-3 py-1.5 border border-white/10 rounded-lg text-xs text-[#8a91a8] hover:text-[#e8eaf0] transition-colors">
            ...
            {/* Report iframe — renders the HTML report */}
