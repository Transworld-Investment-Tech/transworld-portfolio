...
function renderMarkdown(text: string) {
  ...
  return lines.map((line, i) => {
    if (line.startsWith('## '))  return <h2 key={i} style={{ color:'#c9a84c', fontSize:15, fontWeight:700, marginTop:28, marginBottom:8, borderBottom:'1px solid rgba(201,168,76,0.2)', paddingBottom:6, letterSpacing:'0.05em', textTransform:'uppercase' as const }}>{line.slice(3)}</h2>
    ...
    if (line.startsWith('#### '))return <h4 key={i} style={{ color:'#60a5fa', fontSize:12, fontWeight:600, marginTop:14, marginBottom:4 }}>{line.slice(5)}</h4>
    ...
    if (line.startsWith('| ')) {
      ...
      return (
        <div key={i} style={{ display:'flex', borderBottom:'1px solid rgba(255,255,255,0.05)', padding:'6px 0', background: isHeader ? 'rgba(255,255,255,0.03)' : 'transparent' }}>
          {cells.map((c,j) => (
    ...
    if (line.startsWith('- ') || line.startsWith('• ')) {
      const txt = line.replace(/^[-•]\s+/,'').replace(/\*\*(.+?)\*\*/g,'<strong style="color:#e8eaf0;font-weight:600">$1</strong>').replace(/\*(.+?)\*/g,'<em style="color:#8a91a8">$1</em>')
      return <div key={i} style={{ display:'flex', gap:8, margin:'3px 0', paddingLeft:8 }}><span style={{ color:'#555d72', flexShrink:0 }}>·</span><p style={{ fontSize:13, lineHeight:1.7, color:'#8a91a8', margin:0 }} dangerouslySetInnerHTML={{ __html: txt }} /></div>
    }
    const html = line
    ...
    return <p key={i} style={{ fontSize:13, lineHeight:1.8, color:'#8a91a8', margin:'3px 0' }} dangerouslySetInnerHTML={{ __html: html }} />
  }).filter(Boolean)
}
...
function buildPrintHTML(portfolioName: string, reportType: string, reportDate: string, content: string): string {
  ...
  const css = 'body{font-family:Segoe UI,Arial,sans-serif;font-size:10.5pt;line-height:1.7;color:#111}' +
    ...
    'p{margin:5px 0;color:#222}strong{font-weight:700}hr{border:none;border-top:1px solid #ddd;margin:20px 0}' +
  ...
  return '<!DOCTYPE html><html><head><meta charset="UTF-8"><style>' + css + '</style></head><body>' +
    ...
    '<div class="pmeta">' + reportType.toUpperCase() + ' &middot; ' + reportDate + ' &middot; ' + date + '</div></div>' +
      ...
      <div className="w-64 border-r border-white/[0.07] flex flex-col flex-shrink-0 bg-[#13161d]">
        <div className="p-4 border-b border-white/[0.07]">
          ...
          <div className="mb-3 space-y-1.5">
            <div className="flex items-center gap-1 mb-1"><Calendar size={10} className="text-[#555d72]" /><span className="text-[10px] text-[#555d72] uppercase tracking-wider font-semibold">Period</span></div>
            <div><label className="block text-[10px] text-[#555d72] mb-0.5">From</label><input type="date" value={dates.from} onChange={e => setDates(d => ({...d,from:e.target.value}))} className="tw-input py-1 text-xs font-mono" /></div>
            <div><label className="block text-[10px] text-[#555d72] mb-0.5">To</label><input type="date" value={dates.to} onChange={e => setDates(d => ({...d,to:e.target.value}))} className="tw-input py-1 text-xs font-mono" /></div>
          </div>
          ...
          {error && (
            <div className="mt-2 text-[11px] text-[#ff5c7a] bg-[#ff5c7a]/10 rounded px-2 py-2 break-words leading-relaxed">
              ⚠ {error}
            </div>
          )}
          {isLongReport && !generating && (
            <p className="mt-2 text-[10px] text-[#f59e0b] leading-relaxed">
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="px-4 py-2 text-[10px] font-bold uppercase tracking-widest text-[#555d72]">History ({history.length})</div>
          {history.map(r => {
            ...
            return (
              <div key={r.id} onClick={() => setSelectedReport(r)}
                ...
                <div className="flex items-center gap-1 text-[10px] text-[#555d72] mt-0.5">
                  <Clock size={9} />{new Date(r.created_at).toLocaleTimeString('en-GB', { hour:'2-digit', minute:'2-digit' })}
                </div>
              </div>
            )
          })}
        ...
        {generating ? (
          <div className="flex-1 flex flex-col items-center justify-center gap-4">
            <div className="relative">
              ...
              <div className="absolute inset-0 flex items-center justify-center"><Sparkles size={18} className="text-[#a78bfa]" /></div>
            </div>
            <div className="text-center max-w-xs">
              <div className="text-sm text-[#8a91a8] font-medium mb-2">Generating {reportType} analysis…</div>
              <div className="text-xs text-[#555d72] mb-1">Reviewing portfolio history · Scanning watchlist · Building recommendations</div>
              {isLongReport ? (
                <div className="text-xs font-semibold mt-2 px-3 py-1.5 rounded-lg" style={{ color:'#f59e0b', background:'#f59e0b12', border:'1px solid #f59e0b30' }}>
                  {reportType === 'annual' ? '⏳ Annual reports take 90–150 seconds — do not close this tab' : '⏳ Quarterly takes 60–90 seconds — please wait'}
                </div>
              ) : (
                <div className="text-xs text-[#555d72]">Typically 20–40 seconds</div>
              )}
            </div>
          </div>
        ) : selectedReport ? (
          <>
            <div className="px-5 py-2.5 border-b border-white/[0.07] bg-[#13161d] flex items-center justify-between flex-shrink-0">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[11px] capitalize font-medium px-2 py-0.5 rounded" style={{ background:(typeInfo?.color??'#a78bfa')+'18', color:typeInfo?.color??'#a78bfa' }}>{selectedReport.report_type}</span>
                <span className="text-xs text-[#555d72]">{selectedReport.report_date}</span>
              </div>
              <div className="flex items-center gap-2">
                <button onClick={copyReport} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all"
                  style={copied ? { background:'#22c55e18', color:'#22c55e', border:'1px solid #22c55e40' } : { background:'transparent', color:'#8a91a8', border:'1px solid rgba(255,255,255,0.1)' }}>
                  {copied ? <><Check size={12} />Copied!</> : <><Copy size={12} />Copy</>}
                </button>
                <button onClick={printReport} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium" style={{ background:'#a78bfa', color:'#fff' }}>
            ...
            <div className="flex-1 overflow-y-auto px-8 py-6 bg-[#0d0f14]">
              <div style={{ maxWidth:820, margin:'0 auto' }}>
                <div style={{ borderLeft:'3px solid #c9a84c', paddingLeft:16, marginBottom:28 }}>
                  <div style={{ fontSize:10, color:'#c9a84c', letterSpacing:'0.1em', textTransform:'uppercase', fontWeight:700, marginBottom:4 }}>Transworld Asset Management — Portfolio Intelligence</div>
                  <div style={{ fontSize:20, color:'#e8eaf0', fontWeight:700 }}>{portfolio?.name}</div>
                  <div style={{ fontSize:12, color:'#555d72', marginTop:2 }}>{selectedReport.report_type.toUpperCase()} REPORT · {selectedReport.report_date}</div>
                </div>
                ...
                <div style={{ marginTop:40, padding:'12px 16px', background:'#a78bfa0a', border:'1px solid #a78bfa20', borderRadius:8 }}>
                  <div style={{ fontSize:11, color:'#a78bfa', fontWeight:600, marginBottom:4 }}>Tip</div>
                  <div style={{ fontSize:11, color:'#555d72', lineHeight:1.7 }}>Copy and paste into Claude.ai: <em style={{ color:'#8a91a8' }}>"Format this portfolio analysis into a professional PDF-ready report. Keep all analysis and numbers exactly as they are."</em></div>
                </div>
              </div>
            </div>
          </>
