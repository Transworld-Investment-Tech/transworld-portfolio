  ...
  return (
    <div className="min-h-screen bg-[#0d0f14] flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-sm relative">
        ...
        <div className="text-center mb-10">
          <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-[#a78bfa]/10 border border-[#a78bfa]/20 mb-5">
            <svg width="28" height="28" viewBox="0 0 28 28" fill="none">
              ...
              <path d="M7 17 L21 17" stroke="#a78bfa" strokeWidth="2" strokeLinecap="round" opacity=".5"/>
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="you@transworldam.com" required autoFocus
                  className="tw-input"
                />
              </div>
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <button type="button" onClick={() => { setMode('reset'); setError('') }}
                    className="text-[11px] text-[#555d72] hover:text-[#a78bfa] transition-colors">
                    Forgot password?
                  </button>
                </div>
                <input
              ...
              {error && (
                <div className="bg-[#ff5c7a]/10 border border-[#ff5c7a]/20 rounded-lg px-3 py-2.5 text-xs text-[#ff5c7a]">
                  {error}
                </div>
              )}
              <button type="submit" disabled={loading}
                className="w-full py-2.5 bg-[#a78bfa] text-white rounded-lg text-sm font-medium hover:bg-[#9b87e8] disabled:opacity-50 disabled:cursor-not-allowed transition-colors mt-2">
            ...
            <form onSubmit={handleReset} className="space-y-4">
              <div>
                <button type="button" onClick={() => { setMode('login'); setError(''); setResetSent(false) }}
                  className="flex items-center gap-1.5 text-xs text-[#555d72] hover:text-[#e8eaf0] mb-4 transition-colors">
                  ← Back to sign in
                </button>
                {resetSent ? (
                  <div className="bg-[#00d4a4]/10 border border-[#00d4a4]/20 rounded-lg px-4 py-3 text-xs text-[#00d4a4] text-center">
                  ...
                  <>
                    ...
                    <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                      placeholder="you@transworldam.com" required autoFocus className="tw-input" />
                    {error && <div className="text-xs text-[#ff5c7a] mt-2">{error}</div>}
                    <button type="submit" disabled={loading}
                      className="w-full mt-4 py-2.5 bg-[#a78bfa] text-white rounded-lg text-sm font-medium hover:bg-[#9b87e8] disabled:opacity-50 transition-colors">
        ...
        <p className="text-center text-[11px] text-[#555d72] mt-5">
          New user? Contact your administrator to be added.
        </p>
      </div>
    </div>
  )
